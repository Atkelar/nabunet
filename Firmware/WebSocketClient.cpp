...
WebSocketClient::WebSocketClient(bool secure, bool ignoreCertificate) {
  if (secure)
  {
...
bool WebSocketClient::available()
{
...
void WebSocketClient::send(byte *buffer, int length)
{
  DEBUG_WS("[WS] sending bytes");
  if (!client->connected()) {
...
int WebSocketClient::getMessage(byte *buffer, int maxLength)
{
